...
def camshift_tracker(v, file_name):
    ...
    #cv2.circle(frame, (int(c+w/2), r+h/2), 4, (0, 255, 0), 2)
    ...
    while (1):
        ...
        '''if frameCounter%50==0:
            ...
            cv2.circle(frame, (int(ret[0][0]), int(ret[0][1])), 4, (0, 255, 0), 2)
            cv2.imwrite('camshift_'+str(frameCounter)+'.png',frame)'''
        frameCounter = frameCounter + 1
...
def kf_tracker(v, file_name):
    ...
    while (1):
        ...
        if c != 0 and r != 0 and w != 0 and h != 0:  # e.g. face found
            measurement = np.array([c + w / 2, r + h / 2], dtype='float64')
            posterior = kalman.correct(measurement)
            output.write("%d,%d,%d\n" % (frameCounter, posterior[0],
                                         posterior[1]))
...
def pf_tracker(v, file_name):
    ...
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    roihist = hsv_histogram_for_window(frame, (c, r, w, h))
    hist_bp = cv2.calcBackProject([hsv], [0], roihist, [0, 180], 1)
    stepsize = 10
    ...
    while (1):
        ...
        particles = particles.clip(
            np.zeros(2), np.array((v.get(3), v.get(4))) - 1).astype(int)
        ...
        img2 = cv2.rectangle(img2, (pos[0], pos[1]), (pos[0], pos[1]), (255, 0, 0), 8)
        cv2.imwrite('img' + str(frameCounter) + '.png', img2)'''
        frameCounter = frameCounter + 1
    #print particles
    output.close()
...
def of_tracker(v, file_name):
    ...
    feature_params = dict(
        maxCorners=100, qualityLevel=0.2, minDistance=3, blockSize=3)
    ...
    p0 = cv2.goodFeaturesToTrack(
        old_gray[r:r + h, c:c + w], mask=None, **feature_params)
    ...
    while (1):
        ...
        output.write("%d,%d,%d\n" % (framecounter, xmean / count,
                                     ymean / count))
        # cv2.imwrite('frame_' + str(framecounter) + ".png", img)
